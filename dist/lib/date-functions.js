...
export class UnparseableTimestampError extends Error {
    constructor(inputVal = "") {
        super();
        this.name = "UnparseableTimestampError";
        this.message = `Input of ${inputVal} (type ${typeof inputVal}) is not a valid unix timestamp`;
    }
}
export function timestampGuardFormatter(timestamp) {
 ...
 */
export function unixToDateString(unixTimestamp, locale = "en-GB") {
 ...
 */
export function unixToTimeString(unixTimestamp, locale = "en-GB") {
    unixTimestamp = timestampGuardFormatter(unixTimestamp);
    const options = {
        hour: "2-digit",
        hour12: false,
